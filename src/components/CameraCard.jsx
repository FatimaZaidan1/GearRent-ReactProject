...
const CameraCard = ({ camera }) => {
  return (
    <div className="bg-white rounded-xl shadow-lg hover:shadow-2xl transition duration-300 transform hover:-translate-y-1 overflow-hidden cursor-pointer">
      <Link to={`/camera/${camera.id}`} className="block">
        ...
        <div className="p-5">
          ...
          <div className="border-t border-gray-200 pt-4 flex items-center justify-between">
            ...
            {/* Book Now Button */}
            <span className="bg-[#FF8C00] text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-orange-600 transition duration-200 text-sm">
              Book Now
            </span>
          </div>
        </div>
